...
def bytes_to_E2_point(b):
    ...
    if compressed:
        ...
        return points[0] if (larger and select) or (not larger and not select) else points[1]
    return E2(Fp2(int.from_bytes(b[:Lbytes], "big")) * u + int.from_bytes(b[Lbytes:2*Lbytes], "big"),
              Fp2(int.from_bytes(b[2*Lbytes:3*Lbytes], "big")) * u + int.from_bytes(b[3*Lbytes:], "big"))
...
def create_challenge_hash(msg, T1, T2, T3, R1, R2, R3, R4, R5):
    ...
    # FIX: return challenge reduced modulo r (group order)
    c_big = int.from_bytes(h.digest(), "big") % r
    return ZZ(c_big)
